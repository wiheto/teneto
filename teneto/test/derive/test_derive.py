...
def test_slidingwindow_postpro(): 
    ...
    TR_sw_box = teneto.derive.derive(X.transpose(),{'method': 'slidingwindow', 'windowsize': 10,'dimord':'node,time','postpro':'fisher+boxcox+standardize'})
    assert TR_sw_box[0,1,:].std() == 1

def test_taperedslidingwindow(): 
    X = np.random.multivariate_normal([0,0],[[1,0.5],[0.5,1]],20)
    TR_tsw = teneto.derive.derive(X,{'method': 'taperedslidingwindow', 'windowsize': 10,'dimord':'time,node','distribution':'norm','distribution_params':[0,5]})
...
def test_sd(): 
    X = np.random.multivariate_normal([0,0],[[1,0.5],[0.5,1]],20)
    TR_sd = teneto.derive.derive(X.transpose(),{'method': 'spatialdistance','distance':'euclidean','dimord':'node,time'})
    # Perhaps make a better test 
