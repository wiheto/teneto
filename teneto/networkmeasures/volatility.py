...
def volatility(netIn, D='default', do='global', subnetworkID=[]):
    ...
    **HISTORY**
    ...
    if D == 'default' and netInfo['nettype'][0] == 'b':
        ...
        D = 'euclidean'
        print(
            'Default distance funciton specified. As network is weighted, using Euclidean')
    ...
    if do == 'subnetwork':
        ...
        if len(subnetworkID) != netInfo['netshape'][0]:
            raise ValueError(
                'When processing per network, subnetworkID vector must equal the number of nodes')
    ...
    if do == 'global':
        V = np.mean([distanceMetric(netIn[ind[0], ind[1], t], netIn[ind[0],
                                                                    ind[1], t + 1]) for t in range(0, netIn.shape[-1] - 1)])
    elif do == 'time':
        V = [distanceMetric(netIn[ind[0], ind[1], t], netIn[ind[0], ind[1], t + 1])
             for t in range(0, netIn.shape[-1] - 1)]
    # This takes quite a bit of time to loop through. When calculating per edge/node.
        ...
        for i in ind[0]:
            for j in ind[1]:
                V[i, j] = np.mean([distanceMetric(
                    netIn[i, j, t], netIn[i, j, t + 1]) for t in range(0, netIn.shape[-1] - 1)])
        ...
        V = np.zeros([max(netIDs) + 1, max(netIDs) +
                      1, netInfo['netshape'][-1] - 1])
        for net1 in netIDs:
            for net2 in netIDs:
                Vtmp = [distanceMetric(netIn[subnetworkID == net1][:, subnetworkID == net2, t], netIn[subnetworkID ==
                                                                                                      net1][:, subnetworkID == net2, t + 1]) for t in range(0, netIn.shape[-1] - 1)]
                V[net1, net2, :] = Vtmp
    elif do == 'withinsubnetwork':
        within_ind = np.array([[ind[0][n], ind[1][n]] for n in range(
            0, len(ind[0])) if subnetworkID[ind[0][n]] == subnetworkID[ind[1][n]]])
        V = [distanceMetric(netIn[within_ind[:, 0], within_ind[:, 1], t], netIn[within_ind[:, 0],
                                                                                within_ind[:, 1], t + 1]) for t in range(0, netIn.shape[-1] - 1)]
    elif do == 'betweensubnetwork':
        between_ind = np.array([[ind[0][n], ind[1][n]] for n in range(
            0, len(ind[0])) if subnetworkID[ind[0][n]] != subnetworkID[ind[1][n]]])
        V = [distanceMetric(netIn[between_ind[:, 0], between_ind[:, 1], t], netIn[between_ind[:,
                                                                                              0], between_ind[:, 1], t + 1]) for t in range(0, netIn.shape[-1] - 1)]
