...
def volatility(net_in, distance_func_name='default', calc='global', subnet_id=None):
    """
    ...
    **HISTORY**
    ...
    # Get input (C or G)
    net_in, netinfo = utils.process_input(net_in, ['C', 'G', 'TO'])
    ...
    if netinfo['nettype'][1] == 'd':
        ind = np.triu_indices(net_in.shape[0], k=-net_in.shape[0])
    elif netinfo['nettype'][1] == 'u':
        ind = np.triu_indices(net_in.shape[0], k=1)
    ...
    if calc == 'global':
        vol = np.mean([distance_func(net_in[ind[0], ind[1], t], net_in[ind[0],
                                                                       ind[1], t + 1]) for t in range(0, net_in.shape[-1] - 1)])
    elif calc == 'time':
        vol = [distance_func(net_in[ind[0], ind[1], t], net_in[ind[0], ind[1], t + 1])
               for t in range(0, net_in.shape[-1] - 1)]
    # This takes quite a bit of time to loop through. When calculating per edge/node.
    elif calc == 'edge' or calc == 'node':
        vol = np.zeros([net_in.shape[0], net_in.shape[1]])
        for i in ind[0]:
            for j in ind[1]:
                vol[i, j] = np.mean([distance_func(
                    net_in[i, j, t], net_in[i, j, t + 1]) for t in range(0, net_in.shape[-1] - 1)])
        ...
        for net1 in net_id:
            for net2 in net_id:
                vol[net1, net2, :] = [distance_func(net_in[subnet_id == net1][:, subnet_id == net2, t],
                                                    net_in[subnet_id == net1][:, subnet_id == net2, t + 1]) for t in range(0, net_in.shape[-1] - 1)]
        ...
        vol = [distance_func(net_in[within_ind[:, 0], within_ind[:, 1], t], net_in[within_ind[:, 0],
                                                                                   within_ind[:, 1], t + 1]) for t in range(0, net_in.shape[-1] - 1)]
    elif calc == 'betweensubnet':
        between_ind = np.array([[ind[0][n], ind[1][n]] for n in range(
            0, len(ind[0])) if subnet_id[ind[0][n]] != subnet_id[ind[1][n]]])
        vol = [distance_func(net_in[between_ind[:, 0], between_ind[:, 1], t], net_in[between_ind[:,
                                                                                                 0], between_ind[:, 1], t + 1]) for t in range(0, net_in.shape[-1] - 1)]
