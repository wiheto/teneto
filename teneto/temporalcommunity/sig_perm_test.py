...
def sig_perm_test(net,community,T):
    ...
    for i in range(C):
        for j in range(C):
            temp = np.squeeze(Q_rand[:,i,j]) # % squeeze(Qmatrix_r[:,i,j])
    ...
    for iter in range(T):
        q_random = np.squeeze(Q_rand[iter,:,:])
...
def modularity(net,community):
    """
    auxiliary function for 'sig_perm_test'. communityalculates Q
    ...
    for i in range(N):
        for j in range(C):
            tmp[i,j] = np.dot(net[i,:],cl[:,j]) # (1,264) dot (1,264)
