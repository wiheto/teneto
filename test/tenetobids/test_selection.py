...
def test_define_sub_then_task():
    tnet = teneto.TenetoBIDS(teneto.__path__[0] + '/data/testdata/dummybids/', pipeline='fmriprep', bids_tags={'sub':'001'}, raw_data_exists=False)
...
def test_define_run_then_sub():
    tnet = teneto.TenetoBIDS(teneto.__path__[0] + '/data/testdata/dummybids/', pipeline='fmriprep', bids_tags={'run': 'alpha'}, raw_data_exists=False)
...
def test_define_task_then_run():
    tnet = teneto.TenetoBIDS(teneto.__path__[0] + '/data/testdata/dummybids/', pipeline='fmriprep', bids_tags={'task': 'a'}, raw_data_exists=False)
    assert len(tnet.get_selected_files(quiet=1)) == 4
...
def test_get_pipeline_alternatives():
    tnet = teneto.TenetoBIDS(teneto.__path__[0] + '/data/testdata/dummybids/', pipeline='teneto-tests', bids_tags={'task': 'a'}, raw_data_exists=False)
...
def test_set_bad_subjects():
    tnet = teneto.TenetoBIDS(teneto.__path__[0] + '/data/testdata/dummybids/', pipeline='teneto-tests', pipeline_subdir='parcellation', bids_tags={'task': 'a'}, raw_data_exists=False)
